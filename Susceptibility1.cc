...
int main(){
	...
	for (p=0.1; p<0.2; p+=0.1) {
		...
		filename1="v9-l"+ss.str()+"k-susceptibility.txt";
		//filename2="v9-l"+ss.str()+"k-susceptibility-2-3.txt";
		//filename3="v9-l"+ss.str()+"k-susceptibility-3-3.txt";
		//filename4="v9-l"+ss.str()+"k-susceptibility-4-3.txt";
			...
			for (int t=0; t<totaltime; t++) {  //main simulation
				...
				//	double aux4=0;
					...
					for (i=0; i<m; i++) {
						...
						aux2+=(v[i]-aux1)*(v[i]-aux1); // Variance
						
						for (j=0; j<m; j++) {
							aux3+=(v[j]-aux1)*(v[i]-aux1);  // Sum over G4i
				...
				result1<< d << "	" << std::setprecision(10) << X41/(double(time)) << endl;
