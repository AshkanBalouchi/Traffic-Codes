...
int main(){
	...
	double dm=0.09;    //max density 
	double dd=0.0001;    //density step
...
//	int* fourierbit=new int[N*time]; 
	...
	filename3="AveVel-l"+ssl.str()+"k-v"+sss.str()+"-p0.1-"+runnum.str()+".txt";
	std::ofstream resultv(filename3.c_str());   //initialize file
	
	filename4="VelD-l"+ssl.str()+"k-v"+sss.str()+"-p0.1-"+runnum.str()+".txt";
	std::ofstream resultvd(filename4.c_str());   //initialize file
	
	filename1="GapD-l"+ssl.str()+"k-v"+sss.str()+"-p0.1-"+runnum.str()+".txt";
...
/*	fftw_destroy_plan(p1);
	...
	cout <<"l = " << n <<"  num = " << num << "  simulation done!"<<endl;
